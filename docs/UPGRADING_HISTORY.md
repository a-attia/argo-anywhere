# Upgrade history: v1.x → v2.x → v3.0.0

**You probably do not need this document.** It is the archived, blow-by-blow
record of what changed in each release from v1.x through v3.0.0, kept for
provenance and for anyone resurrecting a genuinely old install.

For a current upgrade, start at
[`UPGRADING.md`](UPGRADING.md) — it routes you in two or three steps. For what
shipped in a given release, read [`CHANGELOG.md`](../CHANGELOG.md), which is
the single source of truth for per-release detail from v3.1.0 onward.

Two cautions when reading anything below:

- **Version-pinned.** Every statement describes the release it documents.
  Several have since been superseded — the port-resolution list gained a
  per-user derived default in v3.3.1, and the Claude Code writer switched from
  `ANTHROPIC_AUTH_TOKEN` to `ANTHROPIC_API_KEY` on 2026-07-13. Inline notes
  flag the ones we know about; assume anything unflagged reflects its own era,
  not today.
- **The commands are era-appropriate.** Sections below invoke
  `bash argo-anywhere.sh …`; under the current package install that is
  `argo-anywhere …`.

## Contents

- [Upgrading from v1.x to v2.x](#upgrading-from-v1x-to-v2x)
- [Behavior changes you may notice](#behavior-changes-you-may-notice)
- [Behavior changes in v2.1.0 (Phase 2d defensive-hardening)](#behavior-changes-in-v210-phase-2d-defensive-hardening)
- [Behavior changes in v2.2.0 (Phase 4 multi-tool framework)](#behavior-changes-in-v220-phase-4-multi-tool-framework)
- [Behavior changes since v2.2.0 (landed on `main` for v2.2.1)](#behavior-changes-since-v220-landed-on-main-for-v221)
...
> **Since v3.3.1**, step 5 is a **per-user derived default** — a
> stable port computed from your username within a 500-wide range
> above `64742` — and the bare `64742` is used only when the
> username is unknown. Every install shipping the same literal
> port is what made collisions the expected case on a shared node.
> The v2.2 steps 1–4 are unchanged.
...
enumerated by the disagreement detector. As of 2026-08-12 the
detector covers opencode-global, claudecode-global,
claudecode-project-in-cwd, and **aider-global** (aider shipped in
Phase 5a missing from this enumeration, so an aider config could
sit on a dead port with nothing warning you — fixed). If you use
...
- **The existing CLI surface** for `client` / `setup` / `tunnel` /
  `server` / `status` / `stop` / `update-models` / `list-models` /
  `clean` / `list-tools` / `help` is unchanged; the new `update`,
  `connect`, `configure`, `run`, `install`, and `uninstall` subcommands
  are purely additive. Existing scripts that wrap the script keep working.
...
*Split out of [`UPGRADING.md`](UPGRADING.md) on 2026-08-25, which had grown to
~1000 lines of which the first ~120 were the part anyone still reads. Content
is carried over verbatim; only this header, the contents list, and this footer
are new. Original provenance: created 2026-05-15 by Ahmed Attia (with
substantial AI assistance from Claude per
[`CONTRIBUTORS.md`](../CONTRIBUTORS.md)) as part of Phase 2c+3 of the v2.0
release; revised 2026-05-15 (post-Phase 2d) for the v2.1.0 hardening fixes (M6,
M7, M8, M9, M10, L6, L10); revised 2026-05-18 (Phase 4 / v2.2.0) for the
per-tool scope framework (D-017+D-018+D-019), port-as-state (D-020), OpenCode
project-scope, cross-client port-coherence (D-021), and the `mode_stop`
case-label fix; revised 2026-07-10 (v3.0.0, Model A) for the Python-package
rebuild (D-026..D-029).*
